...
class CampaignBloc extends Bloc<CampaignEvent, CampaignState> {
  ...
  Future<void> _fetchCampaigns(
    ...
    try {
      final campaigns = await getCampaignsUseCase.call();
  ...
  void _filterCampaigns(
    ...
    /// SHOW ALL
    if (event.status == 'All') {
      emit(CampaignLoaded(List.from(allCampaigns)));
    ...
    final filtered = allCampaigns.where((campaign) {
      return campaign.status.toLowerCase().trim() ==
          event.status.toLowerCase().trim();
    }).toList();

    emit(CampaignLoaded(filtered));
  }
}
